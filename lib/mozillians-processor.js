...
let Utils = require('./utils'),
    co = require('co'),
    LocationsProcessor = require('./locations-processor');

const API_KEY = Utils.config.mozillians_api_key,
      DATA_PATH_PUBLIC = './data/generated/mozillians.json';
...
class MozilliansProcessor {
    ...
    constructor() {
        this.mozillians = {
            Public: [],
            Mozillians: [] // this could be used for logged in Mozillians
        };
    ...
    update() {
        MozilliansProcessor.lastRun = Date.now();

        console.info('Update started...');

        this.addDataSources(Utils.config.queries)
            .then(LocationsProcessor.insertMapData)
            .then(json => {
                Utils.saveJsonFileSync(DATA_PATH_PUBLIC, json.Public);
                console.info(`Update finished after ${(Date.now() - MozilliansProcessor.lastRun) / 1000} seconds =)`);
            })
            .catch(console.error);
    }
