...
class MozilliansData {
	...
	constructor() {
		...
		this.queries = utils.config.queries.entries();
		this.query;
	...
	addDataSource() {
		let query = this.queries.next();

		if (query.done) {
			this.globalResolve(this.mozillians);
		} else {
			this.query = query.value[1];
			...
			.then(this.addDataSource.bind(this))
			.catch(this.globalReject);
		}
	}
